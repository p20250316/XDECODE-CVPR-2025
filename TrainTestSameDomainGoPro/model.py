...
class Discriminator(torch.nn.Module):
    def __init__(self, input_dim, num_filter, output_dim):
        ...
        self.conv5 = ConvBlock(num_filter * 8, output_dim, stride=1, batch_norm=False) # activation=False, 
    ...
    def forward(self, x, label):
        ...
        x = self.conv5(x)
        out = torch.sigmoid(x)
        # No use of sigmoid in hinge loss
        return out
...
def add_spectral_norm(module):
    for name, layer in module.named_children():
        if isinstance(layer, torch.nn.Conv2d) or isinstance(layer, torch.nn.ConvTranspose2d):
            module.add_module(name, torch.nn.utils.spectral_norm(layer))
