...
def main():
    ...
    parser.add_argument('--blur_level', type=int, default=19, help='Only three blur levels are available: 19, 25, 29')
    ...
    save_dir = "gopro_test_results_self_blurred_new/"
    model_dir = params.model_dir
    given_blur_level = params.blur_level
    model_dir = model_dir + "BL"+str(given_blur_level)+"/gopro_model/" 
    ...
    generator.cuda()
    generator.eval()
    generator.load_state_dict(torch.load(model_dir + 'generator_param.pkl'))

    
    blur_levels = [given_blur_level] # can be 19, 25, 29
    for blur_level in blur_levels:
        blur_dir = os.path.join(params.dataset_dir, f'blurred_levels/blur_{blur_level}')
        sharp_dir = os.path.join(params.dataset_dir, 'sharp')
        level_save_dir = os.path.join(save_dir, f"blur_level_{blur_level}")
